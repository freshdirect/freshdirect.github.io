    ...
    $(function() {
        ...
        if ($carrousel_container.length && $carrousel_content.length) { // if there are at least one content and one container :)
            ...
            $body.on(transEndEventName, ".slide .carrousel__content", function(e) {
                ...
                $parent.find('.carrousel__content[aria-hidden=true]').addClass('visibility-off').removeClass('current-slide');
                
                // check which slide is currently visible (script.js)
                checkCurrent();

                // prevents pagination dots from being accessible by TABbing (script.js)
                noTabDots();
            ...
            if (hash !== "" && $("#" + hash + ".carrousel__content").length) {
                ...
                // display
                $("#" + hash + ".carrousel__content").removeAttr("aria-hidden").removeClass('visibility-off').addClass('current-slide');
            ...
            $(".carrousel").each(function() {
                ...
                if (carrousel_active_slide !== 0 && carrousel_active_slide < $control_list_links.length && $this.find(".js-carrousel__control__list__link[aria-selected=true]").length === 0) {
                    ...
                    $content_linked.removeAttr("aria-hidden").removeClass('visibility-off').addClass('current-slide');
                    ...
                    // replace .carrouselslide-1-x by .carrouselslide-1-$index_tab
                    $carrousel_container.removeClass(previous_content).addClass(new_content).removeClass('current-slide').trigger("NextSlide");
            ...
            $(".carrousel").each(function() {
                ...
                if ($this.find(".js-carrousel__control__list__link[aria-selected=true]").length === 0) {
                    ...
                    $carrousel_container.trigger('carrousel:slide-changed');
                    $first_content.removeAttr("aria-hidden").removeClass('visibility-off').addClass('current-slide');
        ...
        $body.on("click", ".js-carrousel__control__list__link", function(event, additionnal) {
                ...
                $parent.find('.carrousel__content').removeClass('visibility-off').addClass('current-slide');
                ...
                $tab_linked.removeAttr("aria-hidden").addClass('current-slide');
